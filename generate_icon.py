#!/usr/bin/env python3
"""Generate Lever Torch's mod menu icon: one torch, both of its positions.

Upright is the whole of what a passer-by sees; leaning is what a pull does to
it. Showing them side by side is the mod in one look. Source pixels are read
...
OUT = os.path.join(HERE, "src/main/resources/assets/lever-torch-justfatlard/icon.png")
...
TORCH_LEFT, TORCH_RIGHT = 7, 9
TORCH_TOP, TORCH_BOTTOM = 6, 16
PIVOT_U, PIVOT_V = 8.0, 16.0

ANGLE = math.radians(45)


def stamp_upright(icon, torch, left, top):
    for r in range(TORCH_BOTTOM - TORCH_TOP):
        for c in range(TORCH_RIGHT - TORCH_LEFT):
            px = torch[TORCH_TOP + r][TORCH_LEFT + c]
            if px[3]:
                icon[top + r][left + c] = px


def stamp_leaning(icon, torch, pivot_x, pivot_y):
    """Sampled backwards out of the rotation so every destination pixel takes
    exactly one source pixel and nothing blurs."""
    cos, sin = math.cos(ANGLE), math.sin(ANGLE)
    for y in range(len(icon)):
        for x in range(len(icon[0])):
            ox, oy = x + 0.5 - pivot_x, y + 0.5 - pivot_y
            u = int(PIVOT_U + ox * cos + oy * sin)
            v = int(PIVOT_V - ox * sin + oy * cos)
            if not (TORCH_LEFT <= u < TORCH_RIGHT and TORCH_TOP <= v < TORCH_BOTTOM):
                continue
            px = torch[v][u]
            if px[3]:
                icon[y][x] = px
...
def build_icon():
    """Cobblestone because that is what a lever is bolted to, dark enough that
    two small torches read against it."""
    torch = vanilla("block/torch.png")
    icon = [[shade(px, 0.5) for px in row] for row in vanilla("block/cobblestone.png")]
    stamp_upright(icon, torch, 2, 4)
    stamp_leaning(icon, torch, 7.5, 13.0)
    return scale(icon, 8)
