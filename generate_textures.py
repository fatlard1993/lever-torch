#!/usr/bin/env python3
"""Generate Lever Torch's item sprite: the torch, caught mid-swing.

The placed block is the vanilla torch pixel for pixel, which is the whole point of
it and also the whole problem in an inventory: two identical stacks that behave
differently. So the item alone leans over, at the same 45 degrees the block swings
through when it is pulled. The pixels are vanilla's torch, rotated nearest
neighbour and never smoothed.
...
OUT = os.path.join(HERE, "src/main/resources/assets/lever-torch-justfatlard/textures/item/lever_torch.png")
...
# Half a block's swing, the same 45 degrees the block model turns through, placed
# so the leaning torch sits centred in the 16x16 sprite.
ANGLE = math.radians(45)
PIVOT_X, PIVOT_Y = 4.5, 11.5


def build_item():
    """The vanilla torch, leaning: sampled backwards out of the rotation so every
    destination pixel takes exactly one source pixel and nothing blurs."""
